...
function GenrePage() {
    ...
    const fetchunitTemplate = (filter = {}) => {
        ...
        GenreServices.all({ filter }, unitTemplatePage + 1, unitTemplateLimit).then(res => {
    ...
    const handleDelete = (row = null) => {
        if (window.confirm("Are you sure?")) {
            GenreServices.destroy(row._id).then(res => {
                    ...
                    <Box>
                        <form onSubmit={formik.handleSubmit}>
                            <Grid container spacing={2} direction="row" alignItems="flex-end">
                                ...
                                <Grid item>
                                    <Button color="secondary" variant="contained" type="button" size="small" onClick={() => { handleOpenFormDialog() }}>Add Genre</Button>
                                </Grid>
