...
import * as MembershipServices from "@app/services/admin/settings/membership/MembershipServices";
...
function MembershipPage() {
    ...
    const columns = [
        ...
        {
            field: 'maxDaysForDownloads',
            headerName: 'Max Days For Downloads',
            minWidth: 50,
            align:'center',
            headerAlign: 'center',
            flex: 1,
    ...
    const fetchunitTemplate = (filter = {}) => {
        ...
        MembershipServices.all({ filter }, unitTemplatePage + 1, unitTemplateLimit).then(res => {
    ...
    const handleDelete = (row = null) => {
        if (window.confirm("Are you sure?")) {
            MembershipServices.destroy(row._id).then(res => {
                    ...
                    <Box>
                        <form onSubmit={formik.handleSubmit}>
                            <Grid container spacing={2} direction="row" alignItems="flex-end">
                                ...
                                <Grid item>
                                    <Button color="secondary" variant="contained" type="button" size="small" onClick={() => { handleOpenFormDialog() }}>Add Membership</Button>
                                </Grid>
