...
function SpeakersPage() {
    ...
    const columns = [
        ...
        },
        {
            ...
            headerAlign: 'center',
            flex: 1,
            renderCell: ({ value, row }) => Country[value?.toString()?.toUpperCase()]
        },
        {
            field: 'sex',
            headerName: 'Sex',
    ...
    const fetchunitTemplate = (filter = {}) => {
        ...
        SpeakerServices.all({ filter }, unitTemplatePage + 1, unitTemplateLimit).then(res => {
    ...
    const handleDelete = (row = null) => {
        if (window.confirm("Are you sure?")) {
            SpeakerServices.destroy(row._id).then(res => {
                    ...
                    <Box>
                        <form onSubmit={formik.handleSubmit}>
                            <Grid container spacing={2} direction="row" alignItems="flex-end">
                                ...
                                <Grid item>
                                    <Button color="secondary" variant="contained" type="button" size="small" onClick={() => { handleOpenFormDialog() }}>Add Speakers</Button>
                                </Grid>
