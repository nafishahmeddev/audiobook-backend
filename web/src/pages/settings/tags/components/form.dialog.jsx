...
function FormDialog({ open, row, onClose, onConfirm }) {
    ...
    const handleOnConfirm = (values) => {
        ...
        let res = null;
        if (row) {
            res = TagServices.update(row._id, values)
        } else {
            values.balance = 0;
            res = TagServices.create(values)
        }
            ...
            <Dialog open={open}
                ...
                <FormikProvider value={formik}>
                    <DialogTitle display="flex" alignItems="center">
                        <Box flexGrow={1}>
                           <span style={{fontSize: "18px"}}> {row ? "Update" : "Create"} Tag </span>
                        </Box>
                    </DialogTitle>
                    <DialogContent>
                        <DialogContentText>
                        ...
                        <Grid container spacing={2} rowSpacing={1}>
                            ...
                            <Grid item xs={6}>
                                <Stack direction="row" alignItems="center" spacing={2} sx={{mt: 3}}>
                                    ...
                                    {formik.values.icon && <img src={formik.values.icon} alt="Tag Image" height="300" />}
                                </Stack>
                            </Grid>
