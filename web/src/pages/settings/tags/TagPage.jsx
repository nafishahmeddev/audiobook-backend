...
function TagPage() {
    ...
    const formik = useFormik({
        ...
        validationSchema: yup.object({
            ...
            icon: yup.string("Please select tag cover"),
        }),
        onSubmit: (values) => {
    ...
    const fetchunitTemplate = (filter = {}) => {
        ...
        TagServices.all({ filter }, unitTemplatePage + 1, unitTemplateLimit).then(res => {
    ...
    const handleDelete = (row = null) => {
        if (window.confirm("Are you sure?")) {
            TagServices.destroy(row._id).then(res => {
                    ...
                    <Box>
                        <form onSubmit={formik.handleSubmit}>
                            <Grid container spacing={2} direction="row" alignItems="flex-end">
                                ...
                                <Grid item>
                                    <Button color="secondary" variant="contained" type="button" size="small" onClick={() => { handleOpenFormDialog() }}>Add Tag</Button>
                                </Grid>
