...
router.get("/", async (req: any, res: any) => {
    ...
    return res.status(200).json(ResponseHelper.success({
        ...
        message: "Cool",
        payload: {
            lists: lists.map(list => ({ ...list.toObject(), thumbnail: baseUrl(list.thumbnail) })),
            genres: genres.map(genre => ({ ...genre.toObject(), thumbnail: baseUrl(genre.thumbnail) })),
            authors: authors.map(author => ({ ...author.toObject(), image: baseUrl(author.image) })),
